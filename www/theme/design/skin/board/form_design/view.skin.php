...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_info" style="font-size:14px;">
        <h2>페이지 정보</h2>
        <span class="sound_only">작성자</span>
        <strong><?php echo $view['name'] ?>
          <?php
          if($is_admin){
    ...
    <section id="bo_v_atc">
        ...
        ?>
        <div class="write_div">
          <table style="width:100%; border-bottom:1px solid #e3e3e3; padding:10px 0px; font-size:14px; ">
            <colgroup>
              <col width="150px">
              <col width="">
            </colgroup>
            <tr>
              <td><strong><?=$filed_name_[$i]?></strong></td>
              <td>
                <?php
                if($filed_type_[$i] == 'checkbox'){
                  $check_val = explode('|',$view['wr_'.$i]);
                  for($s = 0; $s < count($check_val)-1; $s++){
                    if($check_val[$s]){
                      $check_value[$i] .= $check_val[$s].', ';
                    }
                  }
                  echo substr($check_value[$i], 0, -2);
                }else{
                  echo $view['wr_'.$i];
                }
                ?>
